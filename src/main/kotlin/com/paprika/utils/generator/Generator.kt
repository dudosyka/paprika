...
abstract class Generator<T : BaseIntEntity>(count: Int, clearOld: Boolean = true, startIndex: Int) {
    protected abstract val generated: List<T>
    protected var sequence: Sequence<Int> = sequenceOf()

    init {
        this.sequence = generateSequence(startIndex) { it + 1 }.takeWhile { it < count }
    }
